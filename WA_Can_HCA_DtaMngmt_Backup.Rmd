...
df$income2 <- factor(df$income2,
                     levels = c(1,2,3,4,5,6,7,8),
                     labels = c("< $10,000", "Less than $15,000",
...
# Creating age label
label(df$X_ageg5yr) <- "Age group (yrs)"

# Creating gender label 
label(df$sex) <- "Gender"

# Creating race label
label(df$X_racegr3) <- "Race/Ethnicity"

# Creating education label
label(df$educa) <- "Education"

# Creating income label 
label(df$income2) <- "Income"

# Creating health insurance label
label(df$hlthpln1) <- "Health insurance status"
...
label(df$curSmk) <- "Smoking status"
...
T1 <- table1(~ X_ageg5yr + sex + X_racegr3 + educa + income2 + hlthpln1 + medcost + ment14d + curAlc + curSmk | mj30day, data=df, overall = "Total")

T1 %>%
  kbl(caption = "Table 1: Characteristics of past month cannabis users") %>%
