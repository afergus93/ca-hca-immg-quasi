...
# Race Factor
df$X_mrace1 <- factor(df$X_mrace1,
                   levels = c(4,2,7,6,3,5,1),
                   labels = c("Asian",
...
# Ethnicity Factor
df$X_hispanc <- factor(df$X_hispanc,
                   levels = c(1,2),
                   labels = c("Yes",
                              "No"),
                   exclude = NULL)
...
# Creating race label
label(df$X_mrace1) <- "Race"

# Creating ethnicity label
label(df$X_hispanc) <- "Hispanic, Latino/a, or Spanish origin"
...
t1 <- tbl_svysummary(design, by = mj30day, 
               include = c(X_age_g, sex, X_mrace1, X_hispanc, edu, X_incomg, hlthpln1, medcost, ment14d, curAlc, curSmk),
